...
are not `is_shown`; these are then left out, so hiding a series shifts the rest rather than leaving a gap.
...
function Validations.validate(context::ValidationContext, data::SeriesBarsGraphData)::Nothing
    ...
    validate_vector_length(context, "order", data.order, "series", length(data.series))
    if !any(series.is_shown for series in data.series)
...
# drawn in, the value axis each of them is drawn on, and the total number of value axes.
struct SeriesArrangement
    series_indices::AbstractVector{<:Integer}
    axis_indices::AbstractVector{<:Integer}
    n_axes::Int
end

# Lay out the series of a graph: reorder them by the `order`, then leave out the ones that are not `is_shown`.
#
# Mirrored, the series are paired by adjacent positions in the order, and the pairing is what `is_shown` may not
# disturb; hiding one side leaves the other where it is, and a pair is left out only when neither of its sides is
# shown. Otherwise each shown series simply takes the next slot.
function series_arrangement(graph::SeriesBarsGraph)::SeriesArrangement
    series = graph.data.series
    ordered_indices = prefer_data(graph.data.order, 1:length(series))
    has_gap = graph.configuration.series_gap !== nothing

    series_indices = Int[]
    axis_indices = Int[]

    if !graph.configuration.mirrored
        for series_index in ordered_indices
            if series[series_index].is_shown
                push!(series_indices, series_index)
                push!(axis_indices, has_gap ? length(series_indices) : 1)
            end
        end
        n_axes = has_gap ? length(series_indices) : 1
    else
        n_pairs = 0
        for pair_position in 1:(length(ordered_indices) ÷ 2)
            sides_indices = (ordered_indices[2 * pair_position - 1], ordered_indices[2 * pair_position])
            if !any(series[side_index].is_shown for side_index in sides_indices)
                continue
            end
            n_pairs += 1
            for (side, side_index) in enumerate(sides_indices)
                if series[side_index].is_shown
                    push!(series_indices, side_index)
                    push!(axis_indices, has_gap ? 2 * (n_pairs - 1) + side : side)
                end
            end
        end
        n_axes = has_gap ? 2 * n_pairs : 2
    end

    return SeriesArrangement(series_indices, axis_indices, n_axes)
end
...
function Common.validate_graph(graph::SeriesBarsGraph)::Nothing
    ...
    if series_value_axis_title(graph) !== nothing &&
       any(graph.data.series[series_index].name !== nothing for series_index in arrangement.series_indices) &&
...
function Common.graph_to_figure(graph::SeriesBarsGraph)::PlotlyFigure
    ...
    show_in_legend =
        any(graph.data.series[series_index].name !== nothing for series_index in series_indices) &&
        graph.configuration.series_gap === nothing
    has_hovers = false

    for (position, series_index) in enumerate(series_indices)
        series = graph.data.series[series_index]
        ...
        scaled_values = push_bar_trace!(;
            traces,
            sub_graph = SubGraph(;
                index = arrangement.axis_indices[position],
                n_graphs = n_axes,
...
function bars_layout(;
    ...
    if graph isa SeriesBarsGraph
        ...
        arrangement = series_arrangement(graph)  # NOJET
        # The series which titles each value axis, if any; mirrored, the hidden side of a shown pair has none.
        series_index_per_axis = Maybe{Int}[nothing for _ in 1:(arrangement.n_axes)]
        for (position, series_index) in enumerate(arrangement.series_indices)
    ...
    if specific_scaled_ranges === nothing
        ...
        for value_axis_position in 1:n_graphs
            axis_index = value_axis_position + n_annotations
            ...
            set_layout_axis!(  # NOJET
                ...
                graph.configuration.value_axis;
                title,
                range = specific_scaled_ranges[value_axis_position],
                domain = plotly_sub_graph_domain(
                    SubGraph(;
                        index = value_axis_position,
                        n_graphs,
                        graphs_gap,
                        mirrored,
                        n_annotations,
